...
- Achei e corrigi um bug novo, isolado: o custo fixo "Bernardo" tinha o vencimento original em setembro E uma entrada duplicada (pagoPorMes) pro mesmo mês, contando a mesma despesa duas vezes. Isso inflava "Custos fixos pagos" de setembro em 1.346,16 euros (de 8.348,16 pro valor correto, 7.002,00). Corrigido direto no banco, e confirmado ao vivo depois: a tela já mostra o valor certo.
- Também limpei duas outras entradas duplicadas (mas inofensivas, com pago:false, não afetavam nenhum total) encontradas na mesma auditoria.
- Removidos todos os travessões (—) dos textos do sistema, 290 ocorrências no código e 269 no changelog, trocados por vírgula, dois pontos ou hífen simples conforme o contexto. Testei tudo de novo depois da troca em massa, nada quebrou.
...
- **Corrigido em dois níveis**: (1) daqui pra frente, marcar um mês projetado como pago grava a data de vencimento daquele mês, não a de hoje; (2) uma correção automática (`applyCorrecaoDatasCustosProjetados`), que roda uma única vez ao abrir o sistema como gestor, resolve todos os registros que já foram gravados errados até agora, sem precisar mexer manualmente em nada.
- Testado o cenário exato visto em produção (CPAS de agosto, marcado pago com a data errada de setembro): depois da correção, agosto passa a mostrar o valor certo e setembro deixa de estar inflado.

## v4.8: Correções de prioridade alta (auditoria detalhada)
- **Bug 1 corrigido, "Custos fixos pagos" mostrando €0,00**: todo o histórico de custos importado (aluguel, CPAS, salários, ChatGPT etc.) tinha `pago:true` mas nunca teve uma data de pagamento registada (esse campo não existia na planilha original). O cálculo de regime de caixa exigia essa data e por isso ignorava esses lançamentos inteiros. Corrigido: quando não há data de pagamento registada, usa a data de vencimento como aproximação, testei com os 5 maiores custos fixos reais do histórico (€5.992,96) e bateu exatamente.
- **Bug 2 investigado, barra da meta "parecendo cheia"**: auditei a fórmula (`valor vendido ÷ meta`) e o CSS da barra a fundo; ambos calculam e desenham corretamente. Não reproduzi esse problema no código atual. Se persistir depois de publicar essa versão, me manda um print novo que eu comparo direto.
- **Bug 3 corrigido, mistura de períodos nos alertas**: "Custos fixos vencidos" agora deixa explícito que é a **situação atual** (hoje), independente do período selecionado na tela, não é mais um número solto sem explicação.
- **Bug 4 corrigido, duplicidade aparente em "Recorrências atrasadas"**: a tabela agora mostra o **serviço** e o progresso de parcelas (ex.: "1/2 pagas") de cada linha, um cliente com dois contratos diferentes agora aparece claramente como duas linhas distintas, não como uma duplicata confusa.
- **Bug 5 testado, herança de status entre meses**: testei rigorosamente (mês seguinte de um custo fixo pago não herda "pago" nenhuma hipótese) e não reproduziu, o comportamento já estava correto, provavelmente era a mesma causa do Bug 1 vista por outro ângulo.
- Removida também `computeGlobal()`, uma função órfã (nunca usada em lugar nenhum) que calculava a mesma meta só que com uma fórmula diferente e inconsistente, resquício que só criava risco de confusão futura.
- Os itens de prioridade média (6-9) e de interface (10-12) da lista ficam registados para as próximas entregas.
