...
- **Comissão** = honorário base recebido (proporcional ao valor já pago) × percentual configurável em Configurações → Regras de comissão.
- **Imposto devido** (IVA + taxa administrativa) é calculado proporcionalmente ao valor **efetivamente recebido** de cada venda/recorrência — não sobre o valor total contratado.
